...
#debug mode flag
#if true, data will not be sent to the server
debug=0
...
class MainPage(webapp2.RequestHandler):
    ...
    def get(self):
        ...
        questions=[]     
        qFile= file("static/questions.txt", "r")
...
class evaluator(webapp2.RequestHandler):
    ...
    def post(self):
        ...
        exhortation=[3, 7, 13, 31, 36, 45, 51, 52, 70, 78, 79, 81, 83, 97, 103]
        giving=[2, 10, 18, 25, 32, 41, 47, 55, 65, 69, 72, 76, 86, 98, 104]
        administration=[8, 14, 20, 23, 29, 35, 44, 49, 56, 71, 73, 82, 93, 95, 102]
        ...
        logging.info(self.request.get("isTakingClass"))
        if (self.request.get("isTakingClass") == '1'):
            takingClass='Yes'
        else:
            takingClass='No'
            
        if(takingClass=='Yes'):
            classDate=self.request.get("classDate")
            #in the future, parse this for date info then reformat for consistency
        else:
            classDate='N/A'
        ...
        if (not debug):  
          response=fusionTables.query().sql(sql="insert into "+tableID+" (TimeStamp, 'First Name', 'Last Name', 'Email Address', 'Taking OBC 301', 'Date Of Class', Prophecy, Service, Teaching, Exhortation, Giving, Administration, Mercy) "+
                                             "values ('"+now.strftime('%c')+"', '"+firstName+"', '"+lastName+"', '"+email+"', '"+takingClass+"', '"+classDate+"', "+str(scores[0])+', '+str(scores[1])+', '+str(scores[2])+', '+str(scores[3])+', '+str(scores[4])+', '+str(scores[5])+', '+str(scores[6])+')').execute(http=http)
